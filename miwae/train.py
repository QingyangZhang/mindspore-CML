...
from mindvision import datasets, transforms
import os
...
def get_mean(loader):
    ...
    for batch_idx, (data, target) in enumerate(loader):
        mean = mindspore.ops.mean(data, axis = 0).unsqueeze(0)
        print(mean.shape)
        var = mindspore.ops.std(data, axis = 0)
        mean_list.append(mean)
        
    total_mean = mindspore.ops.mean(mean_list, 0)
    print(total_mean.shape)
    total_mean_mean = mindspore.ops.mean(total_mean, axis = 0)
...
def save_model(args, model):
    ...
    mindspore.save_checkpoint(model, os.path.join(save_path,f'MIWAE_{str(args.beta)}.pt'))

       
def load_model(args):
    model = MIWAE(args.h, args.d, args.K, args.p, args.classes).cuda()
    # load weights
    model_state_dict=mindspore.load(os.path.join('pts', args.dataset, str(args.seed), f'{str(args.beta)}.pt'))
...
def get_multi_mask(input_num, n, p, feature_per_view):
    ...
    mask = np.ones((n,p))
    mask_list.append(mindspore.Tensor.from_numpy(mask).bool().cuda())
        ...
        for i in range(n):
            for v in range(view_num):
                if mask[i,v] == 0:
                    ...
                    tmp[i,begin:end] = 0
        multi_mask_list.append(mindspore.Tensor.from_numpy(tmp).bool().cuda())
...
if __name__ == "__main__":
    ...
    train_loader = mindspore.utils.data.DataLoader(
        train_dataset, 
    ...
    test_loader = mindspore.utils.data.DataLoader(
        test_dataset, 
        batch_size=args.batch_size,
        shuffle=False,
        **loader_kwargs
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            for i in range(step[2]):
                ...
                for num in range(0,train_input_num):
                    ...
                    _, predict = mindspore.ops.max(probability, 1)
                    confidence = mindspore.ops.max(probability, axis=1)[0]
                ...
                for num in range(train_input_num-1):
                    ...
                    rank_loss += nn.ReLU()(mindspore.ops.Sub(confidence_list[num+1],confidence_list[num])*sign-args.epsilon).sum() 
                    # trick 2
                    ...
                    rank_loss += nn.ReLU()(mindspore.ops.Sub(conf1,conf2)*sign1-args.epsilon).sum()
        ...
        for batch_idx, (data, target) in enumerate(test_loader):
            ...
            for num in range(test_input_num):
                ...
                with F.stop_gradient():
                    ...
                    confidence_list.append(confidence)
                    acc_list[num] = acc_list[num] + mindspore.ops.sum(predict==target.squeeze())/predict.shape[0]
