...
import mindspore.nn.probability.distribution as td
import numpy as np
...
class MIWAE(nn.Module):
    def __init__(self, h, d, K, p, classes):
        ...
        self.decoder = nn.Sequential(
            mindspore.nn.Linear(d, h),
            nn.ReLU(),
            #torch.nn.Linear(h, h),
            #torch.nn.ReLU(),
            nn.Linear(h, 3*p),  # the decoder will output both the mean, the scale, and the number of degrees of freedoms (hence the 3*p)
        )
        ...
        self.encoder = nn.Sequential(
            ...
            #torch.nn.Linear(h, h),
            #torch.nn.ReLU(),
            nn.Linear(h, 2*d),  # the encoder will output both the mean and the diagonal covariance
        )
    ...
    def miwae_loss(self, x, mask):
        ...
        q_zgivenxobs = td.Independent(td.Normal(loc=out_encoder[..., :self.d],\
            scale=nn.Softplus()(out_encoder[..., self.d:(2*self.d)]) + ep),1)
        ...
        all_scales_obs_model = nn.Softplus()(out_decoder[..., self.p:(2*self.p)]) + 0.001
        all_degfreedom_obs_model = nn.Softplus()(out_decoder[..., (2*self.p):(3*self.p)]) + 3
  
        data_flat = mindspore.Tensor.repeat(iota_x,[self.K,1]).reshape([-1,1])
        tiledmask = mindspore.Tensor.repeat(mask,[self.K,1])
        ...
        logpxobsgivenz = mindspore.ops.sum(all_log_pxgivenz*tiledmask,1).reshape([self.K,batch_size])
        logpz = p_z.log_prob(zgivenx)
        logq = q_zgivenxobs.log_prob(zgivenx)
  
        neg_bound = -mindspore.ops.mean(torch.logsumexp(logpxobsgivenz + logpz - logq,0))
        #print(neg_bound)
    ...
    def miwae_impute(self, x, mask, L):
        ...
        p_z = td.Independent(td.Normal(loc=torch.zeros(self.d).cuda(), scale=torch.ones(self.d).cuda()), 1)
        out_encoder = self.encoder(iota_x)
        if mindspore.any(mindspore.isnan(out_encoder)):
            print('nan')
        if mindspore.any(torch.isnan(x)):
            print('NAN')
        q_zgivenxobs = td.Independent(td.Normal(loc=out_encoder[..., :self.d],\
            scale=nn.Softplus()(out_encoder[..., self.d:(2*self.d)]) + ep),1)
        ...
        data_flat = mindspore.Tensor.repeat(iota_x,[L,1]).reshape([-1,1]).cuda()
        tiledmask = mindspore.Tensor.repeat(mask,[L,1]).cuda()
  
        all_log_pxgivenz_flat = td.StudentT(loc=all_means_obs_model.reshape([-1,1]),
        ...
        logpxobsgivenz = mindspore.ops.sum(all_log_pxgivenz*tiledmask,1).reshape([L,batch_size])
        logpz = p_z.log_prob(zgivenx)
        logq = q_zgivenxobs.log_prob(zgivenx)
  
        xgivenz = td.Independent(td.StudentT(loc=all_means_obs_model, scale=all_scales_obs_model, df=all_degfreedom_obs_model),1)

        imp_weights = nn.functional.softmax(logpxobsgivenz + logpz - logq,0) # these are w_1,....,w_L for all observations in the batch
        xms = xgivenz.sample().reshape([L,batch_size,self.p])
        xm=mindspore.einsum('ki,kij->ij', imp_weights, xms) 
    ...
    def miwae_mse(self, data, target):
        ...
        imputed_data = data
        loss_fn = mindspore.nn.MSELoss(reduction='mean')
